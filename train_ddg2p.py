...
def main():
    ...
    params = {'model': config['model']['model'], 'd_model': config['model']['d_model'], 'n_layers': config['model']['n_layers']}
    ...
    train_dataloader = DataLoader(train_subset, batch_size=config['data']['batch_size'], shuffle=True, collate_fn=pad_collate)
    ...
    if profile:
        ...
        trainer = G2pTrainer(model, train_dataloader, optimizer, device, config['model']['PATH'], test_subset=test_subset)
